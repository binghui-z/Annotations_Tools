...
class pose_annotation_app:
    ...
    def init_variables(self):
        ...
        self.img_left, self.img_center, self.img_right, self.mask = None, None, None, None
    ...
    def init_window(self):
        ...
        window_width = params.IMG_SIZE*4 + params.IMG_PADDING*5
    ...
    def init_frame_info(self):
        ...
        self.mask = cv2.resize(self.img_left, dsize=(params.IMG_SIZE, params.IMG_SIZE))
    ...
    def init_image_display(self):
        self.img_display_np = np.ones((params.IMG_SIZE+params.IMG_PADDING*2, \
            (params.IMG_SIZE+params.IMG_PADDING*2)*4, 3), dtype=np.uint8)*128
        ...
        self.window.canvas = tk.Canvas(self.window.panel_image_display, width=params.IMG_SIZE*4 + params.IMG_PADDING*6, \
    ...
    def init_model_segnet(self):
        ...
        model = HandSegNet_ScaleUp__Deploy()
        pretrained_mask_path = params.MODEL_MASK_PATH
        if not os.path.exists(pretrained_mask_path):
            self.model_mask = None
            print('Model mask not found : {}'.format(pretrained_mask_path))
        else:
            print('Loading {}'.format(pretrained_mask_path))
            print('model_2d #params: {}'.format(sum(p.numel() for p in model.parameters())))
            model.load_state_dict(torch.load(pretrained_mask_path))
            self.model_mask = model.cuda()
            self.model_mask.eval()
            print('Model mask succesfully loaded')
    ...
    def in_img_right_range(self, row, col):
        if (row >= params.IMG_PADDING and row < params.IMG_SIZE + params.IMG_PADDING) and \
            (col >= params.IMG_PADDING*5+params.IMG_SIZE*2 and col < params.IMG_PADDING*5+params.IMG_SIZE*4):
            return True
        else:
            return False
    ...
    def update_img_display(self):
        ...
        row_s_m = row_s_l
        row_e_m = row_e_l
        col_s_m = col_e_r + params.IMG_PADDING*2
        col_e_m = col_s_m + params.IMG_SIZE
        
        # cv2.imshow("mask",self.mask)
        # cv2.waitKey(-1)
        self.img_display_np[row_s_m:row_e_m, col_s_m:col_e_m, :] = self.mask.copy()
    ...
    def button_pred_mask_callback(self):
        if self.model_mask is None:
            ...
            img_resized = cv2.resize(self.img_left, (params.CROP_SIZE_PRED, params.CROP_SIZE_PRED)).astype(np.float32) / 255.0
            img_input_tensor = torch.from_numpy(img_resized - 0.5).permute(2,0,1).unsqueeze(0).cuda()
            mask_pred = self.model_mask(img_input_tensor)
            mask_result = mask_pred[0].cpu().data.numpy()[0]

            mask_result = cv2.resize(mask_result,dsize=(params.IMG_SIZE,params.IMG_SIZE))
...
if __name__ == '__main__':
    args = parse()
    # samples_dir_path=r"\\105.1.1.1\Hand\Hand3D\FreiHand\downloads\evaluation\rgb"
